$(function() {
	...
	$('#contact-form').submit(
		function(e) {
			...
			$.ajax({
				url : 'https://formspree.io/xzbgbjko',
				method : 'POST',
...
function setSkillProgress(skill){
	
	var target = $('#skill-progress');
	target.empty();
	
	$.each(skill, function(i, v){
		var div1 = $("<div>", {"class": "bar flex"});
		var div2 = $("<div>", {"class": "bar fill", style: "width:"+v.per+"%"});
		var div3 = $("<div>", {"class": "tag bold flex", text: v.name});
		var span = $("<span>", {text: v.per+"%"});
		
		target.append(div1);
		div1.append(div2).append(span);
		div2.append(div3);
	})
}
